...
public class T01_FileTest {
	public static void main(String[] args) throws IOException {
		...
		// => 중간의 경로가 모두 미리 만들어져 이어야 한다.

		// 2. mkdirs() => 중간의 경로가 없으면 중간의 경로도 새롭게 만든 후 마지막 위치의 디렉토리를 만들어 준다.

		// => 위 메서드는 ㅗㅁ두 만들기를 성공하면, true, 실패하면 false를 반환함
		...
		File file6 = new File("d:/D_Other/test/java/src"); // mkdir => 중간 경로가 존재하지 않아 false 반환
		if (file6.mkdir()) {
		...
		File file7 = new File("d:/D_Other/test/java/src"); // mkdirs => 중간 경로가 없어도 모두 만들주어 true 반환
		if (file6.mkdirs()) {
			System.out.println(file6.getName() + "만들기 성공 !");
		} else {
			System.out.println(file6.getName() + "만들기 실패 !");
